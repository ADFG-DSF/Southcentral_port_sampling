...
ggplot() +
  ...
  theme_bw(base_size = 16) +
  labs(title = expression(paste("GOA Yelloweye Rockfish rate to Approach ", L[infinity])))
```
...
rflgage %>%
  ...
  ggplot(aes(x = spp2, y = length)) +
    geom_boxplot() +
    geom_text(data = lgplot_names, aes(x = spp2, Inf, label = format(Freq, digits = 0, big.mark = ",")), vjust = 1) +
    labs(x = "Species", y = "Length") +
    theme_bw(base_size = 16) +
    theme(axis.text.x = element_text(angle = -20, vjust = 0.5, hjust=0.5)) 
```
  
```{r, fig.height = 7, fig.width = 8.5, fig.cap = "R package ielationship between weight and length for primary rockfish species, 1996-2019."}
