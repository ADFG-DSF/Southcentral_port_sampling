...
data spcomp_init;
	...
	*Get at these data by SFmgmtarea;
	length SFmgmtarea $5.;
	*PWS;
		if statarea > 440000 and statarea < 480000 or statarea in(485430,485500,485530,485600,
		485630,485700,485730,485800,485831,485901,485931,485932,485935,486001,486002,486003,
		486004,486005,486031,486032,486033,486034,486100) then SFmgmtarea = 'PWS';
	*North Gulf;
	else if statarea in(485832,485902,485933,485934,485935,486002,495831,495901,495902,
		...
		505831,505901,505902,505903,505904,505905,505906,505907,505908,505909,505931,505932 
		 505933,505934) then SFMgmtarea = 'NG';
	*Kodiak;
	else if statarea in(495800,495832,505700,505730,505800,505832,515630,515700,515730
		...
		555701,555733) then SFmgmtarea = 'KOD';
	*AKPen;
	else if statarea in(555731,555732,545731,545801,545802,545803,535801,535832)
		then SFmgmtarea = 'AKPen';
	*CI;
	else if statarea in(515831,515832,515901,515902,515903,515904,515905,515906,515907,
		...
		535931,	535932,535933,545900) then SFmgmtarea = 'CI';
	if statarea = . then do; *Starts a 'do loop' that will define the mgmt area of blank stat areas;
	if port = 'Homer' then SFmgmtarea = 'CI';
	if port = 'Kodiak' then SFmgmtarea = 'KOD';
	if port = 'Whittier' then SFmgmtarea = 'PWS';
	if port = 'Valdez' then SFmgmtarea = 'PWS';
	end; *Remember to close a 'do loop' with end statement;
	if SFmgmtarea = '' then delete;
...
proc sort data=spcomp;
	by SFmgmtarea user year species;
proc freq data=spcomp;
	by SFmgmtarea user year species;
  tables abbrev/noprint out=comp;  *{output is SFmgmtarea year species count percent};
data comp; set comp;
	drop percent;
*proc print data=comp noobs uniform;
*	title 'work.comp: obs freq by species and SFmgmtarea';
run;
...
data comp3;
	...
	drop _type_ _freq_;
	n = sum(nC,nP,nU,nM);
	piC = niC/nC;										*{proSFmgmtareaion of species i in charter sample};
	vpiC = piC*(1-piC)/(nC-1);
	piP = niP/nP;
	vpiP = piP*(1-piP)/(nP-1);
...
Filename pharv DDE 'Excel|[RFHarv9619SFmgmtarea.xlsx]Sheet1!R2C1:R97C8'; 
data pharv;
	infile pharv DLM='09'x NOTAB DSD MISSOVER;
	informat SFmgmtarea $CHAR8. year 4. HC 12. SEHC 12. HP 12. SEHP 12. H 12. SEH 12.;
 	input SFmgmtarea year HC SEHC HP SEHP H SEH;
	vHC = SEHC**2;
	vHP = SEHP**2;
	vH = SEH**2;
	drop SEHC SEHP SEH;
...
data comp4;
	...
	pi2 = ni/n;
	*{calculations for all SFmgmtareas/years except Seward 1996-2000};
	if SFmgmtarea ne 'NG' or (SFmgmtarea = 'NG' and year ge 2001) then do;
	...
	*{Calculations for Seward 1996-2000};
	if SFmgmtarea = 'NG' and year ge 1996 and year le 2000 then do;
...
proc print data=comp4 noobs uniform;
	...
	var SFmgmtarea year species n niC niP niU niM ni piC vpiC piP vpiP HC HP H Hi SEHi HiC SEHiC HiP SEHiP pi SEpi;
...
proc sort data=plotcomp;
	by SFmgmtarea year rank;
title;
proc sgplot data=plotcomp noautolegend;	
	*title 'Cook Inlet RF species comp';
	*by SFmgmtarea;
	*where SFmgmtarea = 'CI'; *{EDIT THIS LINE TO PRODUCE PLOTS WITHOUT SFmgmtarea NAMES};
...
data comp4_tpi;
	set comp4_Tpi;
	drop _name_;
title 'Species comp by SFmgmtarea (exclude blank columns for appendix table';
proc print data=comp4_Tpi noobs uniform;
	by SFmgmtarea;
	var year SFmgmtarea BGL BLK BOC CAN CHI COP DRK DUS DUSDRK HAR NOR QUI REYE ROS RSTP SGRY SHCN SRAK TIG VER WID YEYE YTAL;
...
data comp4_tSEpi;
	set comp4_TSEpi;
	drop _name_;
title 'Standard errors of species comp by SFmgmtarea (exclude blank columns for appendix table';
